...
#[Route('/gandaal/administrationnnnn/scolarite/pdf')]
class PdfController extends AbstractController
{
    ...
    public function carteScolaire(Etablissement $etablissement, InscriptionRepository $inscriptionRep, ClasseRepartitionRepository $classeRep, PersonnelRepository $personnelRep, ConfigFonctionRepository $fonctionRep, Request $request, SessionInterface $session): Response
    {
        ...
        foreach ($inscriptions as $inscription) {
            ...
            if (!file_exists($photoPath)) {
                // Si la photo n'existe pas, définir la photo par défaut
                $photoPath = $this->getParameter('kernel.project_dir') . '/public/images/config/default.jpg';
            }
    ...
    { 
        ...
        // return $this->render('gandaal/administration/scolarite/pdf/carte_retrait.html.twig', [
        //     'logoPath' => $logoBase64,
        //     'symbolePath' => $symboleBase64,
        //     'ministerePath' => $ministereBase64,
        ...
        //     'responsable' => $responsable,
        // ]);
        $html = $this->renderView('gandaal/administration/scolarite/pdf/carte_retrait_pdf.html.twig', [                     
            'logoPath' => $logoBase64,
            'symbolePath' => $symboleBase64,
            'ministerePath' => $ministereBase64,
            'signaturePath' => $signatureBase64 ? $signatureBase64 : 'nok',
    ...
    #[Route('/carte/retrait/simple/{etablissement}', name: 'app_gandaal_administration_scolarite_pdf_carte_retrait_pdf_simple')]
    public function carteRetraitSimple(Etablissement $etablissement, InscriptionRepository $inscriptionRep, ClasseRepartitionRepository $classeRep, PersonnelRepository $personnelRep, ConfigFonctionRepository $fonctionRep, Request $request, SessionInterface $session): Response
    { 
        $search = $request->get('search');
        $inscriptions = $request->get('inscription') ? $inscriptionRep->findBy(['id' => $request->get('inscription')]) : null;
    ...
    #[Route('/liste/eleve/classe/{etablissement}/{classe}', name: 'app_gandaal_administration_scolarite_pdf_liste_eleve_classe')]
    public function ListeEleveClasse(ClasseRepartition $classe, InscriptionRepository $inscriptionRep, Etablissement $etablissement, TrieService $trieService, SessionInterface $session ): Response
    {       
        $logoPath = $this->getParameter('kernel.project_dir') . '/public/images/config/'.$etablissement->getEntreprise()->getLogo();
        $logoBase64 = base64_encode(file_get_contents($logoPath));
        $symbolePath = $this->getParameter('kernel.project_dir') . '/public/images/config/symbole.png';
        ...
        // dd($inscriptions);
        $html = $this->renderView('gandaal/administration/pedagogie/pdf/liste_eleve_classe.html.twig', [                     
            'logoPath' => $logoBase64,
            'symbolePath' => $symboleBase64,
            'ministerePath' => $ministereBase64,
            'etablissement' => $etablissement,
            'inscriptions' => $inscriptions,
            'classe_repartition' => $classe,
        ...
        return new Response($dompdf->output(), 200, [
            'Content-Type' => 'application/pdf',
            'Content-Disposition' => 'inline; filename=liste_eleve_'.$classe->getNom().date("d/m/Y à H:i").'".pdf"',
        ]);
    }

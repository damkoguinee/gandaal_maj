...
class InscriptionController extends AbstractController
{
    ...
    public function new(Request $request, Etablissement $etablissement, ClasseRepartitionRepository $classeRepartitionRep, SessionInterface $session, FraisScolariteRepository $fraisScolRep, FraisInscriptionRepository $fraisInsRep, PaiementEleveRepository $paiementRep, InscriptionRepository $inscriptionRep, EleveRepository $eleveRep, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $verif = $inscriptionRep->findOneBy(['eleve' => $eleveRep->find($request->get('id_eleve')), 'promo' => $session->get('promo')]);
            if ($verif) {
                $this->addFlash('danger', 'Cet élève est déjà inscrit pour cette année-scolaire');
    ...
    #[Route('/confirm/delete/general/{id}/{etablissement}', name: 'app_gandaal_administration_scolarite_inscription_confirm_delete', methods: ['GET', 'POST'])]
    public function confirmDeleteGeneral(Inscription $inscription, Request $request, Etablissement $etablissement): Response
    {
        ...
        $route_suppression = $this->generateUrl('app_gandaal_administration_scolarite_inscription_delete', [
            'id' => $inscription->getId(),
            'origine' => 'eleve',
        ]);
        

        return $this->render('gandaal/administration/scolarite/inscription/confirm_delete.html.twig', [
            'inscription' => $inscription,
            'etablissement' => $etablissement,
    ...
    #[Route('/{id}', name: 'app_gandaal_administration_scolarite_inscription_delete', methods: ['POST', 'GET'])]
    public function delete(Request $request, Inscription $inscription, SessionInterface $session, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete'.$inscription->getId(), $request->getPayload()->getString('_token'))) {
            ...
            $historique->setType('inscription') // ou un type plus spécifique
                ...
                ->setPromo($session->get('promo'))
                ->setUser($inscription->getEleve());
            $entityManager->persist($historique);
            $entityManager->remove($inscription);
            $entityManager->flush();
            $this->addFlash("success", 'inscription annulée avec succèes :)');
        }

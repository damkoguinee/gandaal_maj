...
class EleveController extends AbstractController
{
    ...
    public function index(EleveRepository $eleveRepository, InscriptionRepository $inscriptionRep, SessionInterface $session, Request $request, Etablissement $etablissement): Response
    {
        ...
        if ($session->get('session_cursus')) {
            $inscriptions = $inscriptionRep->listeDesElevesInscritParCursusParPromoParEtablissement($session->get('session_cursus'), $session->get('promo'), $etablissement, $search, $pageEnCours, 100);
    ...
    {
        if ($session->get('session_cursus')) {
            $cursus = $cursusRep->findOneBy(['id' => $session->get('session_cursus')]);
    ...
    public function new(Request $request, UserRepository $userRep, ClasseRepartitionRepository $classeRepartitionRep, EntityManagerInterface $em, EleveRepository $eleveRep, UserPasswordHasherInterface $userPasswordHasher, SessionInterface $session, FiliationRepository $filiationRep, EtablissementRepository $etablissementRep, PersonnelRepository $personnelRep, Etablissement $etablissement): Response
    {
       ...
       // Initialiser les objets nécessaires
        ...
        if ($session->get('session_cursus')) {
            $classes = $classeRepartitionRep->listeDesClassesParCursusParPromo($session->get('session_cursus'), $session->get('promo'));
    ...
    public function newInscription(Request $request, EntityManagerInterface $entityManager, UserPasswordHasherInterface $userPasswordHasher, SessionInterface $session, InscriptionRepository $inscriptionRep, ClasseRepartitionRepository $classeRepartitionRep, HistoriqueSuppressionRepository $historiqueSuppressionRep, EleveRepository $eleveRep,  Etablissement $etablissement): Response
    {
        ...
        if ($form_eleve->isSubmitted()) {
            ...
            $maxInscription = $inscriptionRep->findCountInscriptionId('inscription', $etablissement, $session->get('promo')) ?:0;
            
            $nbre_sup_inscription = $historiqueSuppressionRep->findCountSup('inscription', $etablissement, $session->get('promo')) ?:0;
            $formattedMaxInscription = sprintf('%04d', ($maxInscription + $nbre_sup_inscription + 1)); 
            ...
            if ($session->get('lien')) {
                ...
                $lien_familial->setLienParent('frère/soeur');
                $lien_familial->addEleve($frere);
                $lien_familial->addEleve($eleve_ins);
                $entityManager->persist($lien_familial);
            }
            $verif_eleve = $eleveRep->findOneBy(['nom' => $eleve_ins->getNom(), 'prenom' => $eleve_ins->getPrenom(), 'dateNaissance' => $eleve_ins->getDateNaissance(), 'lieuNaissance' => $eleve_ins->getLieuNaissance(), 'telephone' => $eleve_ins->getTelephone()]) ;
            if ($verif_eleve) {
                $this->addFlash('danger', $verif_eleve->getNomComplet().' est déjà enregistré');
                ...
                $session->remove('lien');
                return $this->redirectToRoute('app_gandaal_administration_scolarite_eleve_new', ['etablissement' => $etablissement->getId()], Response::HTTP_SEE_OTHER);
            ...
            // if ($frere) {
            //     $newInscription = $inscriptionRep->find($id);
            //     foreach ($newInscription->getEleve()->getFiliations() as $key => $value) {
            //         $entityManager->remove($value);
            //     }

            //     $ajusteFiliation = new Filiation();

            //     dd($ajusteFiliation);
            //     dd($newInscription, $filiation, $frere);
            // }
    ...
    #[Route('/confirm/delete/eleve/{id}/{etablissement}', name: 'app_gandaal_administration_scolarite_eleve_confirm_delete', methods: ['GET', 'POST'])]
    public function confirmDeleteGeneral(Inscription $inscription, Request $request, Etablissement $etablissement): Response
    {
        $param = $request->request->get('param'); // Récupération du paramètre
        $route_suppression = $this->generateUrl('app_gandaal_administration_scolarite_eleve_delete', [
            'id' => $inscription->getId(),
            'etablissement' => $etablissement->getId(),
            'origine' => 'eleve',
        ]);
        
        return $this->render('gandaal/administration/scolarite/eleve/confirm_delete.html.twig', [
            'inscription' => $inscription,
            'etablissement' => $etablissement,
    ...
    public function delete(Request $request, Inscription $inscription, SessionInterface $session, EntityManagerInterface $entityManager, Etablissement $etablissement): Response
    {
        if ($this->isCsrfTokenValid('delete'.$inscription->getId(), $request->getPayload()->getString('_token'))) {

            if ($inscription->getEleve()->getPhoto()) {
                $ancienFichier=$this->getParameter("dossier_eleves")."/".$inscription->getEleve()->getPhoto();
                if (file_exists($ancienFichier)) {
                    unlink($ancienFichier);
                }
            }
            $deleteReason = $request->request->get('delete_reason');
            $information = $inscription->getEleve()->getNomComplet().' matrcule '.$inscription->getEleve()->getMatricule();
            $historique = new HistoriqueSuppression();
            $historique->setType('inscription') // ou un type plus spécifique
            ...
            $entityManager->remove($inscription->getEleve());
            $entityManager->flush();
            $this->addFlash("success", 'élève annulé avec succèes :)');
        }

...
class FraisScolariteController extends AbstractController
{
    ...
    public function new(Request $request, CursusRepository $cursusRep,  FormationRepository $formationRep, EntityManagerInterface $entityManager, FraisScolariteRepository $fraisScolariteRep, SessionInterface $session, Etablissement $etablissement): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
           ...
           foreach ($formations as $value) {
                ...
                $verif_frais = $fraisScolariteRep->findOneBy(['formation' => $formation, 'tranche' => $form->getViewData()->getTranche(), 'promo' => $session->get('promo'), 'etablissement' => $etablissement]);
                if (!$verif_frais) {
                    $fraisScolarite = new FraisScolarite();
                    ...
                    $this->addFlash("warning", "Frais de scolarité déja ajouté ");
                }

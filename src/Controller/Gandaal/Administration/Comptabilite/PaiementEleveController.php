...
class PaiementEleveController extends AbstractController
{
    ...
    public function new(Request $request, Etablissement $etablissement, ConfigCompteOperationRepository $compteOpRep, ConfigCategorieOperationRepository $categorieOpRep, MouvementCaisseRepository $mouvementCaisseRep, InscriptionRepository $inscriptionRep, UserRepository $userRep, SessionInterface $session, FraisScolariteRepository $fraisScolRep, FraisInscriptionRepository $fraisInsRep, PaiementEleveRepository $paiementRep, TranchePaiementRepository $tranchePaieRep, EtablissementRepository $etablissementRep, ConfigCaisseRepository $caisseRep, ConfigModePaiementRepository $modePaieRep, HistoriqueSuppressionRepository $historiqueSuppressionRep, ConfigDeviseRepository $deviseRep, EntityManagerInterface $entityManager): Response
    {
        ...
        $nbre_sup_inscription = $historiqueSuppressionRep->findCountSup('paiement frais scolarité', $etablissement, $session->get('promo')) ?:0;

        $formattedMaxPaie = sprintf('%04d', $maxPaie + $nbre_sup_inscription + 1);
        $generatedReference = $currentYear . $formattedMaxPaie;

        $referenceExists = $paiementRep->findOneBy(['reference' => $generatedReference]);

        if ($referenceExists) {
            // Si la référence existe déjà, ajouter uniqid pour garantir l'unicité
            $uniqid = uniqid();
            $reference = $generatedReference . $uniqid;
        } else {
            // Si la référence n'existe pas, l'utiliser telle quelle
        ...
        if ($request->get('paiement_eleve')) {
            ...
            if ($type == 'inscription') {// gestion des paiements des frais inscriptions
                
                $reste_a_payer = $reste_inscription;
                $montant_paye = floatval(preg_replace('/[^0-9,.]/', '', $request->get('montant')));
                $epsilon = 0.00001;
                if (($montant_paye - $reste_a_payer) > $epsilon) {
            ...
            if (intval($type) == $type) {// on gère les tranches individuellement
                ...
                if (($montant_paye - $reste_a_payer) > $epsilon) {
                    ...
                    // Vérifiez si l'élève existe déjà dans le panier
                    $eleveExisteDeja = false;
                    $fraisScolId = $frais_tranche_paye->getId();
                    // dd($panier);
                    foreach ($panier as &$paiement) {                        
                        // dd($panier, $paiement, $fraisScolId);
                        if ($paiement['inscriptionEleve']->getEleve()->getId() === $search->getId() && ($paiement['fraisScol'] ? $paiement['fraisScol']->getId() == $fraisScolId : 1==2)) {
                            // dd($panier, $paiement['fraisScol']->getId(), $fraisScolId);
        ...
        if ($request->get('finaliser')) {
            ...
            foreach ($panier as $item) {  
                ...
                if ($item['fraisScol']) {
                    ...
                    // dd($nbre_sup_inscription, $maxPaie , $reference, $paiement_eleve_scol);
        ...
        $sessionModePaie = $session->get('modePaie') ? $session->get('modePaie') : ($session->get('configLogiciel') ? ($session->get('configLogiciel')->getModePaieDefaut() ? $modePaieRep->find($session->get('configLogiciel')->getModePaieDefaut()) : []) : []);

        $sessionCaisse = $session->get('caisse') ? $session->get('caisse') : ($session->get('configLogiciel') ? ($session->get('configLogiciel')->getCaissedefaut() ? $caisseRep->find($session->get('configLogiciel')->getCaisseDefaut()) : []) : []);
        ...
        return $this->render('gandaal/administration/comptabilite/paiement_eleve/new.html.twig', [
            ...
            'session_modePaie' => $sessionModePaie,

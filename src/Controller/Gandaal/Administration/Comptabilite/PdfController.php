...
#[Route('/gandaal/administrationnnnn/comptabilite/pdf')]
class PdfController extends AbstractController
{
    ...
    {       
        ...
        $donnees = [];
        if ($classe) {
            $inscriptions = $inscriptionRep->listeDesElevesInscritActifParPromoParEtablissementParClassePaginated($session->get('promo'), $etablissement, $classe, $search, $pageEncours, 20000);

        }else{

            $inscriptions = $inscriptionRep->listeDesElevesInscritActifParPromoParEtablissement($session->get('promo'), $etablissement, $search, $pageEncours, 20000);
        }
    ...
    {       
        ...
        foreach ($inscriptions as $inscription) {
            ...
            foreach ($cumulPaiements as $cumul) {
                $typePaie = $cumul['paiement']->getTypePaie(); // "1ère tranche", "2ème tranche", etc.
                $montant = (float)$cumul['paiement']->getMontant(); // Convertir en float pour additionner

                // Ajouter le montant au total pour ce type de tranche
                if (!isset($paiements_tranche[$typePaie])) {
                    $paiements_tranche[$typePaie] = 0;
                }
                $paiements_tranche[$typePaie] += $montant;

                // Ajouter le montant au total des paiements
            ...
            if ($reste_scolarite_tranche) {
                $donnees [] = [
                    ...
                    'cumuls' => $cumulPaiements,
                    'paiement' => $paiement,
                    'remise' => $remiseScolarite,
                    'paiements_tranche' => $paiements_tranche,
                ];
            }
        } 
        ...
        // return $this->render('gandaal/administration/comptabilite/pdf/relance_scolarite.html.twig', [
        //     'logoPath' => $logoBase64,
        //     'symbolePath' => $symboleBase64,
        //     'ministerePath' => $ministereBase64,
        //     'etablissement' => $etablissement,

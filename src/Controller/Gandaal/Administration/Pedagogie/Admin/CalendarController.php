...
class CalendarController extends AbstractController
{
    ...
    public function index(Etablissement $etablissement, PersonnelActifRepository $personnelActifRep, ClasseRepartitionRepository $classeRep, SessionInterface $session, Request $request): Response
    {
        $classe = $request->query->get('classe') ? $classeRep->find($request->query->get('classe')) : null;
        $enseignant = $request->query->get('enseignant') ? $personnelActifRep->findOneBy(['personnel' => $request->query->get('enseignant'), 'promo' => $session->get('promo')]) : null;
        ...
        if ($session->get('session_cursus')) {
            $enseiganants = $personnelActifRep->listeDesEnseignantsActifParCursusParPromo($session->get('session_cursus'), $session->get('promo'));

        }else{
            $enseiganants =  $personnelActifRep->listeDesEnseignantsActifParEtablissementParPromo($etablissement, $request->getSession()->get('promo'));
        }

        return $this->render('gandaal/administration/pedagogie/admin/calendar/index.html.twig', [
            'etablissement' => $etablissement,
            'enseignants' => $enseiganants,
            'classes' => $classes,
            'classe' => $classe,
    ...
    public function new(Request $request, UserRepository $userRep, PersonnelActifRepository $personnelActifRep, ClasseRepartitionRepository $classeRep, MatiereRepository $matiereRep, SessionInterface $session, EntityManagerInterface $entityManager, Etablissement $etablissement): Response
    {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            if ($session->get('session_cursus')) {
                $personnels = $personnelActifRep->rechercheUserParCursusParPromo($search, $session->get('session_cursus'), $session->get('promo'));
